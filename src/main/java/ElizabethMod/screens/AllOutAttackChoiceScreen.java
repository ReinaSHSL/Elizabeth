...
public class AllOutAttackChoiceScreen extends CardRewardScreen {
    ...
    public AbstractCard allOutAttackChoice;

    public AllOutAttackChoiceScreen() {
        this.codexCard = null;
    ...
    private void acquireCard(final AbstractCard hoveredCard) {
        this.allOutAttackChoice = hoveredCard;
    }
    ...
    public void open(final ArrayList<AbstractCard> cards, final RewardItem rItem, final String header) {
        AbstractDungeon.cardRewardScreen = ElizabethModInitializer.aoas;
        this.codex = false;
        this.draft = false;
        this.rItem = rItem;
        this.skipButton.hide();
        ...
        this.placeCards(Settings.WIDTH / 2.0f, AllOutAttackChoiceScreen.CARD_TARGET_Y);
    }
    ...
    private void placeCards(final float x, final float y) {
        switch (this.rewardGroup.size()) {
            ...
            case 3: {
                this.rewardGroup.get(0).target_x = Settings.WIDTH / 2.0f - AbstractCard.IMG_WIDTH - AllOutAttackChoiceScreen.PAD_X;
                this.rewardGroup.get(1).target_x = Settings.WIDTH / 2.0f;
                this.rewardGroup.get(2).target_x = Settings.WIDTH / 2.0f + AbstractCard.IMG_WIDTH + AllOutAttackChoiceScreen.PAD_X;
                this.rewardGroup.get(0).target_y = y;
                this.rewardGroup.get(1).target_y = y;
            ...
            case 4: {
                this.rewardGroup.get(0).target_x = Settings.WIDTH / 2.0f - AbstractCard.IMG_WIDTH * 1.5f - AllOutAttackChoiceScreen.PAD_X * 1.5f;
                this.rewardGroup.get(1).target_x = Settings.WIDTH / 2.0f - AbstractCard.IMG_WIDTH / 2.0f - AllOutAttackChoiceScreen.PAD_X / 2.0f;
                this.rewardGroup.get(2).target_x = Settings.WIDTH / 2.0f + AbstractCard.IMG_WIDTH / 2.0f + AllOutAttackChoiceScreen.PAD_X / 2.0f;
                this.rewardGroup.get(3).target_x = Settings.WIDTH / 2.0f + AbstractCard.IMG_WIDTH * 1.5f + AllOutAttackChoiceScreen.PAD_X * 1.5f;
                this.rewardGroup.get(0).target_y = y;
                this.rewardGroup.get(1).target_y = y;
                this.rewardGroup.get(2).target_y = y;
    ...
    static {
        ...
        TEXT = AllOutAttackChoiceScreen.uiStrings.TEXT;
